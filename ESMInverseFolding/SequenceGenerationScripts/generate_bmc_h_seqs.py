...
def generate_sequences(rank, world_size):
    ...
    for i in range(num_sequences):
        ...
        sequence_id = f"BMC_H__{i+1 +(num_sequences * (rank))}"

        record = SeqRecord(Seq(sampled_sequence), id=sequence_id, description="")
        sampled_sequences.append(record)

    outfile = SeqIO.write(sampled_sequences, f"BMC_H_GPU{rank}.fasta", "fasta")
    ...
    outfile = pickle.dump(scores, open(f"BMC_H_GPU{rank}_scores.pkl", "wb"))
