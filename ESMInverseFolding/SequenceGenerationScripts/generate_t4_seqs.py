...
def generate_sequences(rank, world_size):
    ...
    for i in range(num_sequences):
        ...
        sequence_id = f"T4_{i+1 +(num_sequences * (rank-1))}"

        record = SeqRecord(Seq(sampled_sequence), id=sequence_id, description="")
        sampled_sequences.append(record)

    outfile = SeqIO.write(sampled_sequences, f"T4_GPU{rank}.fasta", "fasta")
    ...
    outfile = pickle.dump(scores, open(f"T4_GPU{rank}_scores.pkl", "wb"))
